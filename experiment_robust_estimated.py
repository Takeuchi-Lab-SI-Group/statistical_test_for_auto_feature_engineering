...
def run_experiment(seed, n, d):
    ...
    # estimated sigma
    y = rng.normal(size=(n,))
    residuals = y - X @ np.linalg.pinv(X.T @ X) @ X.T @ y
    sigma = np.std(residuals, ddof=X.shape[1])
    cov = np.eye(len(y)) * sigma ** 2
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=96) as executor:
        res = executor.map(run_experiment, list(range(0,10000)), [args.n]*10000, [args.d]*10000)
    ...
    result_path = "summary_pkl/estimated"

    if not os.path.exists(result_path):
        os.makedirs(result_path)

    file_name = f"n_{args.n}_d_{args.d}_estimated_si.pkl"
    file_path = os.path.join(result_path, file_name)
    with open(file_path, 'wb') as f:
        pickle.dump(experiment_results_si, f)

    file_name = f"n_{args.n}_d_{args.d}_estimated_ds.pkl"
    file_path = os.path.join(result_path, file_name)
    with open(file_path, 'wb') as f:
